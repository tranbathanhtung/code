(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(require('@workspace/sdk')) :
  typeof define === 'function' && define.amd ? define(['@workspace/sdk'], factory) :
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.sdk));
})(this, (function (sdk) { 'use strict';
  		      ...
  		      if (renderer.Mount && renderer.Mount._renderNewRootComponent) {
  		        ...
  		        if (renderRootCode.indexOf('should be a pure function') !== -1) {
  		          ...
  		          if (renderRootCode.indexOf('development') !== -1) {
  		            return 'development';
  		          } // I've seen "development" !== 'production' being smartly
  ...
  const checkDevtoolsGlobalHook = () => window.__REACT_DEVTOOLS_GLOBAL_HOOK__ &&
  ...
  const findFiberByHostInstance = (target) => {
      if (!checkDevtoolsGlobalHook())
          return null;
  ...
  const getInspectName = (element) => {
      ...
      if (!fiber)
          return "Source code could not be identified.";
      const { fileName, columnNumber, lineNumber } = fiber._debugSource;
      const path = (fileName || "").split("/");
      return `${path.at(-3) || ""}/${path.at(-2) || ""}/${path.at(-1)}:${lineNumber || 0}:${columnNumber || 0}`;
  };
  function initializeDevToolsLatest() {
      ...
      const handleInspectorClick = async (e) => {
          ...
          target.id = tmpId;
          connection.selectElement(fiber._debugSource);
      };
  }
